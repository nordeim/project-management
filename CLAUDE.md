...
5. **VERIFY** — Run the full gate: `bun run lint && bun run typecheck && bun run test && bun run build && ./scripts/smoke-test.sh` (93 unit + 30 smoke checks required).
...
- **Test at the pure seams.** Router mapping, clarify questions, plan sanitization, check-in mapping, auth rate limiting, team-form normalization, the dashboard's next-planned-action derivation, the logo dot geometry, the date-picker calendar grid + long date format (`formatLongDate`), and the activity feed's date grouping (`groupActivityByDate`) live in `src/lib/*.ts` (or `logo.tsx`) with Vitest specs — TDD (red → green) is the default for changes there.
...
- **The visual system is neumorphic (measured, v1.4–v1.6) with THREE card tiers.** Tokens and primitive classes in `globals.css`: `.orb-raised` / `.orb-raised-lg` / `.orb-raised-btn` / `.orb-well` / `.orb-well-pill` / `.orb-task-blocked` / `.orb-btn-dark` / `.orb-btn-post` / `.orb-btn-cancel` / `.orb-pill-round` / `.orb-row-card` (deeper r14) / `.orb-goal-card` (deeper r16) / `.orb-panel` (large r16). Tiers (measured on the live app): standard pair `-5px 0.78` (buttons), deeper pair `-5px 0.92` (row cards), large pair `-8px 0.31` (view panels). **Never pair an arbitrary `shadow-[…]`, `rounded-*`, or `h-*` utility with a custom class that sets that property** (`.orb-card`/`.orb-raised`/`.orb-btn-*`) on the same element — custom layer classes are emitted after generated utilities and win the cascade; use a dedicated custom class for overrides (`.orb-task-blocked`, `.orb-btn-post` exist for exactly this).
- **In-dialog primary submits are dark charcoal pills** (`#3A3A3A`/`#2F2823` bg + near-white text); page-level actions stay neumorphic raised. The shell clamps all main content to `max-w-[1200px]` over a canvas washed with a fixed purple radial glow; don't add per-view max-widths. The dashboard is a 2×2 grid (date card + 180px CSS-circle ring card (p-6) | stats panel; activity + goals below — 20px gaps, `.orb-panel` tier, activity card p-0 with full-width rows and 30px solid icon circles, goals card p 20/18/0 with inset-well rows + 60px rings); the check-in modal is 448px/radius 16 while form dialogs stay 500px/radius 20. Task cards are flat radius-14 deeper-tier rows (p 14/18, gray 11px/500 status, 14px/500 titles, 11px meta, solid AI chip) with 26px action squares pinned outside the card. My Tasks ships five filter tabs (no Need Help); empty states render directly on the canvas. The desktop sidebar is STICKY (clock always visible); mobile ships an app bar (logo + user pill) plus a floating rounded-24 pill tab bar with inset active highlights.
...
- Manual QA matrix: every changed dialog must be exercised in both desktop and mobile layouts (floating pill tab bar + app bar + MORE sheet below `lg`).
...
bun run test        # 93 unit checks must pass
