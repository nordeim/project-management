...
| 📊 **Dashboard** | 2×2 grid: date card + inset DONE circle beside the stats panel, Agent Activity and Goals panels below — large-tier embossed cards on a 1200px clamped canvas with a soft purple glow |
| 🪝 **Sticky desktop sidebar** | The sidebar pins to the viewport — the analog clock and TASKS STATUS card stay visible while the main content scrolls |
...
| 📜 **Agent activity feed** | Every mutation logs a typed, human-readable activity entry; the full view groups entries under date labels ("Thu Jul  16 2026") inside per-day cards, with the latest entry as a hero card |
| 🎨 **Neumorphic design system** | Measured three-tier token system from the reference — beige canvas `#EBE7E2`, standard/deeper/large embossed shadow pairs for buttons, row cards and view panels, inset wells for inputs/chips/clock, `#DDD8D0` progress tracks, coral inset ring on blocked task cards |
...
| 📱 **Responsive SPA** | Sticky collapsible desktop sidebar (240px ⇄ 64px icon rail) with live analog clock; mobile chrome mirrors the reference — a top app bar (logo + user pill), side-by-side date/ring hero, and a floating rounded-24 pill tab bar (Home / Goals / My Tasks / Agent / More) with a MORE sheet and abbreviated stat labels |
...
Measured from the reference app (v1.6) — a soft-beige neumorphic system with **three card tiers**: raised surfaces carry dual embossed shadows (light `rgba(255,250,244,…)` top-left, dark `rgba(160,143,126,…)` bottom-right) at standard depth for buttons, deeper alpha for row cards (goals/tasks/activity), and a large `-8px` pair for view panels (dashboard, settings, stats); inset wells carry the same pair inverted. Main content sits in a 1200px clamped column over a canvas washed with a soft purple radial glow bottom-right; every in-dialog primary submit is a charcoal pill (`#3A3A3A` bg, `#F1F1F0` text); the desktop sidebar is sticky; mobile ships an app bar + floating pill tab bar.
