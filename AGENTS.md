...
| Unit tests (93 checks) | `bun run test` |
...
- **The dashboard is a 2×2 grid** (v1.5/v1.6): top-left cell = date card (bright `day-hills.jpg` photo, 0.8 opacity, raised date square radius 12) beside a 180×180 ring card (p-6, inner CSS circle 156px, NO SVG ring; `clamp(28px,3.5vw,52px)` fw-300 numerals); top-right = the three-column stats panel (`.orb-panel`, p 20/24, columns p 10/8); bottom = Agent Activity (card p-0; header px 18/pt 14; NPA well inset 14px margins; full-width rows p 14/18 with 30px solid icon circles `#C9B3F5`/`#2ECC8A` + 13px `square-check-big` glyphs) + Goals (card p 20/18/0; rows as inset wells p 12/14 with 60px SVG rings). Grid gap 20px; the greeting is 28px at every breakpoint.
- **The desktop sidebar is STICKY** (v1.6, measured): the `aside` pins at `top-0` with height `calc(100vh-3rem)` so the clock / TASKS STATUS / collapse control stay visible while main scrolls; the bottom block uses `mt-auto`. Task cards (v1.6): radius-14 deeper-tier cards (`p 14px 18px`) holding only flat content — gray 11px/500 status chip + colored dot, 14px/500 title (coral when blocked), 12px one-line description, 11px meta with 11px lucide icons, a solid `#EEEAE6` AI chip (r6, 10px/500 `#996CE4`) — with 26px radius-7 raised action squares pinned top-right OUTSIDE the shadowed card; the row click opens the check-in modal. The activity feed (v1.6): most recent entry as a standalone hero card, then date-grouped rows (uppercase 10px/600 labels, `groupActivityByDate` in the tested `src/lib/activity-groups.ts` seam) each wrapped in one big radius-14 card.
...
- **Pure domain seams are unit-tested** (`src/lib/router.ts`, `clarify.ts`, `plan-sanitizer.ts`, `checkin.ts`, `rate-limit.ts`, `team.ts`, `next-action.ts`, `calendar.ts`, `activity-groups.ts` + the logo geometry helpers in `logo.tsx`, all with Vitest `*.test.ts` — 93 checks). Route handlers import these modules instead of inlining the logic — extend the tests when you extend the logic.
...
- **Mobile navigates with a floating pill tab bar + app bar** (v1.6): below `lg` a sticky full-bleed app bar (ORBITAL logo + user pill, 62px, bottom drop shadow) replaces the desktop greeting header, the date card + ring sit side-by-side, and the bottom tab bar is a FLOATING rounded-24 raised pill with the active tab on an inset well highlight — not a full-width flat bar. The MORE sheet (Tasks / Team / Settings) stays. Desktop keeps the sidebar.
