...
| Unit tests (80 checks) | `bun run test` |
...
- **The visual system is neumorphic, built from measured tokens** (v1.4): canvas `#EBE7E2`, raised surfaces `#EEEAE6` (dual embossed shadows), inset wells `#EBE7E2` (inverted pair), progress track `#DDD8D0`. Primitive classes live in `globals.css` under `@layer utilities`: `.orb-raised` / `.orb-raised-lg` (panels), `.orb-raised-btn` (buttons), `.orb-well` / `.orb-well-pill` (insets), `.orb-card` (alias), `.orb-task-blocked` (blocked-task coral ring). **Cascade warning:** custom classes in that layer are emitted AFTER Tailwind's generated utilities, so an arbitrary `shadow-[…]` utility on an element that also carries `.orb-card`/`.orb-raised` LOSES — style shadow overrides as dedicated custom classes (that's why `.orb-task-blocked` exists). Pairing `bg-orb-well` (a color utility) with `shadow-[…]` is fine.
- **Dialogs are 500px neumorphic panels** (radius 20, `#EEEAE6`): the base `DialogContent` in `ui/dialog.tsx` carries `sm:max-w-[500px] rounded-[20px]` — don't add `rounded-3xl`/`max-w-md` overrides per dialog (that was the pre-v1.4 look).
- **The date picker is a custom component** (`ui/date-picker.tsx`): well-style trigger → popover calendar (Su–Sa headers, 7×6 grid, today ringed). Grid math lives in the pure seam `src/lib/calendar.ts` (`monthGrid`, `isSameDay`).
...
- **Pure domain seams are unit-tested** (`src/lib/router.ts`, `clarify.ts`, `plan-sanitizer.ts`, `checkin.ts`, `rate-limit.ts`, `team.ts`, `next-action.ts`, `calendar.ts` + the logo geometry helpers in `logo.tsx`, all with Vitest `*.test.ts` — 80 checks). Route handlers import these modules instead of inlining the logic — extend the tests when you extend the logic.
