...
> Distilled from sessions 1–18 (v1.0 → v2.3) of cloning and remediating the
...
| Browser tests | Playwright | 1.63 | 26 checks, `tests/e2e/*.spec.ts`, own port + scratch DB |
...
the pipeline carry `ai: true` and render the solid `#EEEAE6` AI chip.
...
bun run test:e2e       # 26 Playwright checks (needs the build)
```
...
| Range | Chrome | Notes |
|---|---|---|
| < 768 (`md`) | Mobile: app bar 62px (logo + user pill, sticky, bottom shadow) + full-width bottom tab bar (`rounded-t-[20px]`, upward shadow, pad 8/8/12; every tab = flex-column chip gap 4 pad 8/4 r14; ACTIVE = `.orb-nav-active` inset well; MORE never welled) + MORE bottom sheet (r24, pad 20/20/40, 20% + 4px-blur scrim) | Full-bleed; main 16/6/90 + per-view containers (dashboard +16/+12 → hero x22/y90; lists +12/+24 → h1 x18/y102) |
...
| Browser tests | `tests/e2e/*.spec.ts` (26) — scratch DB `db/e2e.db`, port 3100 |
